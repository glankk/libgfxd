...
The following functions can be used for registering argument callbacks;
...
```
Select `ucode` as the target microcode. `ucode` can be on of the following:
`gfxd_f3d`, `gfxd_f3db`, `gfxd_f3dex`, `gfxd_f3dexb`, or `gfxd_f3dex2`. The
microcode must always be selected before `gfxd_execute`, as no microcode is
selected by default.

```
void gfxd_endian(int endian)
```
Select `endian` as the endianness of the input. `endian` can be one of the
following: `gfxd_endian_big`, `gfxd_endian_little`, or `gfxd_endian_pdp`.
Big endian is selected by default.
...
is a union of type `gfxd_value_t` with the following
layout;
```
typedef union
{
